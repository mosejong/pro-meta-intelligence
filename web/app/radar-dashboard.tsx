...
function RadarDashboardContent({ initialSpace = "ONBOARDING" }: { initialSpace?: ProductSpace }) {
  ...
  return (
    <main className={`section-space space-${initialSpace.toLowerCase()} ${viewMode === "QUICK" ? "quick-view" : "full-view"}`}>
      ...
      {initialSpace === "TEAM" && <PlayerPracticePanel key={selectedMyTeam?.team_id ?? "no-team"} ownTeam={selectedMyTeam} opponent={selectedOpponent} reviewCandidates={teamBrief.map((card) => card.entry)} />}
